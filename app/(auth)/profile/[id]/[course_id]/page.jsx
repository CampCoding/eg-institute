...
export default function EnhancedCourseDetailPage() {
  ...
  const liveClasses = [
    ...
    },
    {
      ...
      isPopular: false,
 recordingUrl:"/images/Day 1 - P3 (HTML Basic elements).mp4",    }
  ];
  ...
  const tabs = [
    { id: 'lessons', label: 'Lessons', icon: PlayCircle, count: lessons.length },
    { id: 'lives', label: 'Live Classes', icon: Video, count: liveClasses.filter(c => c.status === 'upcoming').length },
    { id: 'books', label: 'Books', icon: BookOpen, count: books.length },
    { id: 'exams', label: 'Exams', icon: PenTool, count: exams.filter(e => e.status === 'available').length }
  ];
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'lessons':
        return (
         <ProfileCourseLessons lessons={lessons} course={course} />
        );
        
      case 'lives':
        return (
         <ProfileCoursesLives liveClasses={liveClasses}/>
        );
        
      case 'books':
        return (
         <ProfileCoursesBooks animateProgress={animateProgress} books={books}/>
        );
        
      case 'exams':
        return (
         <ProfileCoursesExams getDifficultyColor={getDifficultyColor} exams={exams}/> 
        );
          ...
          <div className="max-w-7xl mx-auto w-full">
            ...
            <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
              <div className="flex-1">
                ...
                <p className="text-2xl text-white/90 mb-6 font-medium">{course.subtitle}</p>
                <p className="text-lg text-white/80 mb-8 leading-relaxed max-w-2xl">{course.description}</p>
            ...
            <div className="p-10">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-3xl font-black text-gray-800">Ready to Learn?</h2>
                </div>
                <button 
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-2xl font-bold text-gray-800">{currentLesson.title}</h3>
                  <span className={`px-4 py-2 rounded-full text-sm font-bold border ${getDifficultyColor(currentLesson.difficulty)}`}>
                    {currentLesson.difficulty}
                  </span>
                </div>
                <p className="text-gray-600 mb-6 text-lg leading-relaxed">{currentLesson.description}</p>
                ...
                <div className="flex items-center gap-6 mb-8">
                  <div className="flex items-center gap-3 bg-gradient-to-r from-[#02AAA0]/10 to-[#02AAA0]/5 rounded-2xl px-5 py-3 border border-[#02AAA0]/20">
                    ...
                    <span className="font-semibold text-gray-700">{currentLesson.duration}</span>
                  </div>
                  <div className="flex items-center gap-3 bg-gradient-to-r from-[#C9AE6C]/10 to-[#C9AE6C]/5 rounded-2xl px-5 py-3 capitalize border border-[#C9AE6C]/20">
                    ...
                    <span className="font-semibold text-gray-700">{currentLesson.type} Lesson</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-[#02AAA0]/5 to-[#C9AE6C]/5 rounded-3xl p-8 mb-8 border border-[#02AAA0]/10">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="font-bold text-gray-800 text-lg">What You'll Learn</span>
                  </div>
                  <p className="text-gray-700 leading-relaxed mb-4">Master essential shopping vocabulary, learn bargaining techniques, and practice real-world conversations in Egyptian markets.</p>
                  <div className="flex flex-wrap gap-2">
                    <span className="px-3 py-1 bg-white/60 rounded-full text-sm font-medium text-gray-700 border">Vocabulary Building</span>
                    <span className="px-3 py-1 bg-white/60 rounded-full text-sm font-medium text-gray-700 border">Cultural Context</span>
                    <span className="px-3 py-1 bg-white/60 rounded-full text-sm font-medium text-gray-700 border">Practical Conversation</span>
                  </div>
                </div>
              </div>
