...
const ArabicLearningLanding = () => {
  ...
  return (
    <div className=" bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-12 px-4 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
            ...
            >
              ...
              <div
                className={`relative h-full bg-gradient-to-br ${
                  plan.gradient
                } backdrop-blur-sm border border-white/20 rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 group ${
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="mb-8">
                    <button
                      className={`w-full py-4 px-6 rounded-xl font-bold text-sm tracking-wide transition-all duration-300 transform hover:scale-105 hover:-translate-y-1 ${plan.buttonStyle}`}
                    >
                      {plan.buttonText}
                    </button>
                  </div>
                  ...
                  <div className="space-y-4">
                    {plan.features.map((feature, featureIndex) => (
                      <div
                        key={featureIndex}
                        className={`flex items-center space-x-3 p-3 rounded-xl bg-white/40 backdrop-blur-sm transition-all duration-300 hover:bg-white/60 hover:transform hover:translate-x-2 ${
                          hoveredCard === plan.id ? "animate-slide-in" : ""
                        }`}
                        style={{ animationDelay: `${featureIndex * 100}ms` }}
                      >
                        <div
                          className={`flex-shrink-0 w-8 h-8 rounded-lg bg-white/60 flex items-center justify-center ${plan.iconColor}`}
                        >
        ...
        {/* <div className={`text-center mt-16 transform transition-all duration-1000 delay-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-teal-500 to-blue-600 text-white px-8 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 cursor-pointer">
